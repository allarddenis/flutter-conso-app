...
import 'package:conso/model/Vehicle.dart';
import 'package:conso/ui/DateTimeItem.dart';
...
class DataDialogState extends State<DataDialog> {
  ...
  void getVehicles() async {
    VehicleDao.getAll().then((sqlData){
      vehicles = sqlData;
      setState((){
  ...
  void _submit() {
    ...
    if (form.validate()) {
      ...
      if(action == 'Update') DataDao.update(dataSubmitted);
      else if(action == 'New') DataDao.add(dataSubmitted);
      Navigator.of(context).pop(true);
    } else {}
  }

  _deleteData(){
    DataDao.delete(data).then((val)=>
      Navigator.of(context).pop(true)
    );
  }
  ...
  Widget _buildAddUpdateElement(){
    final text = (action != null && action == 'Update') ? 'Validate data' : 'Add data';
    return new GestureDetector(
      onTap: () => _submit(),
      child: new Container(
        margin: const EdgeInsets.only(top: 50.0, left: 25.0, right: 25.0),
        child: new Container(
          padding: const EdgeInsets.all(20.0),
          decoration: new BoxDecoration(
            color: Colors.brown[100].withOpacity(0.80),
            borderRadius: new BorderRadius.circular(10.0)
          ),
          child: new ListTile(
            leading: new CircleAvatar(
